...
def running_median(datx,daty,bin_size=30,shuffle=5):
    j = 0
    xvals = []
    yvals = []
    while True:
...
def smooth(filename,path,mask=None):
    ...
    #####
    # Move all of the above to an outside function and have only (x,y,redshift,mask) as input
    #####
	...
	mad = np.average(np.abs(np.median(linear_y)-linear_y),weights=np.divide(err,np.sum(err)))
	ransac = linear_model.RANSACRegressor(random_state=0,loss='absolute_loss',residual_threshold=2.0*mad)
	ransac.fit(x.reshape(len(x),1), linear_y,sample_weight=np.abs(err))
